...
poper_help : dialog {
    ...
    : boxed_column {
        ...
        : text { value = \"Настройки:\\n- Масштаб по высоте/длине: сколько метров чертежа = 1 м реальному\"; width = 75; height = 5; }
        : text { value = \"- Округление, единица уклона, цвет проекции и т.д.\\n- Показать длины/уклоны/знак промилле/высота из текста\"; width = 75; height = 5; }
    }
    ok_button;
}")
        ...
        (progn
            ...
            (cond
                ...
                ((= mode_points "1")
                    ...
                    (if (>= point_count 2)
                        (create-polyline (vl-sort PCoords '(lambda (a b) (< (car a) (car b)))) "PoperSwift" 251)
                    )
                )
                ;; Режим: По векторам
                ((= mode_vectors "1")
                ...
                ((= mode_polyline "1")
                    (while (null profile_polyline)
                        ...
                        (setq profile_polyline (car (entsel)))
                        (if (null profile_polyline)
                            (princ "\nПолилиния не выбрана. Пожалуйста, выберите полилинию.")
                            (if (not (member (cdr (assoc 0 (entget profile_polyline))) '("LWPOLYLINE" "POLYLINE")))
                                (progn
                                    (princ "\nВыбранный объект не является полилинией.")
                                    (setq profile_polyline nil)
                    ...
                    (setq axis_line (entmakex (list '(0 . "LINE")
                                                    (cons 10 (list min_x (cadr OSZ) 0.0))
                                                    (cons 11 (list max_x (cadr OSZ) 0.0)))))
                    ...
                    (if int_points
                        (progn
                            ...
                            (while (< i (length int_points))
                                (setq pt (list (nth i int_points) (nth (+ i 1) int_points) (nth (+ i 2) int_points)))
                                (setq param (vlax-curve-getParamAtPoint profile_polyline_obj pt))
                                (if (and param (>= param 0) (<= param (vlax-curve-getEndParam profile_polyline_obj)))
                                    (progn
                                        (setq is_duplicate nil)
                                        (foreach existing_pt PCoords
                                            (if (and (< (abs (- (car pt) (car existing_pt))) 0.001)
                                                     (< (abs (- (cadr pt) (cadr existing_pt))) 0.001)
                                                     (< (abs (- (caddr pt) (caddr existing_pt))) 0.001))
                                                (setq is_duplicate t)
                                            )
                                        )
                                        (if (not is_duplicate)
            ...
            (if (and draw_projections (or (= mode_points "1") (= mode_polyline "1")))
                (progn
                    ...
                    (while (null (setq projection_point (getpoint)))
                        (princ "\nТочка не выбрана. Пожалуйста, выберите точку."))
                    (setq line_color (if (= mode_polyline "1")
                                        (vlax-get (vlax-ename->vla-object profile_polyline) 'Color)
                                        (if (= projection_color 0) 256 projection_color)))
                    (foreach pt original_PCoords
                        (create-line (car pt) (cadr pt) (car pt) (cadr projection_point) line_color)
                    )
                )
            )
            ;; Размещение текста высоты
...
(princ "\nSwift POPER v3.1 (DCL встроен) загружен. Команда: POPER")
